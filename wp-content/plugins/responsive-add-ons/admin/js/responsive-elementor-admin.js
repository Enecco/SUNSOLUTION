	...
	ResponsiveElementorSitesAdmin = {
				...
				if (add_section_tmpl.length > 0) {
					...
					elementor.on("preview:loaded", function () {
						...
						if ($('#rst-sites-modal').length == 0) {
							...
							$elscope = $('#rst-sites-modal');
							$elscope.find('.responsive-sites-content-wrap').before(header_template);
						...
						// Other events.
						$elscope.find('.responsive-sites-content-wrap').scroll(ResponsiveElementorSitesAdmin._loadLargeImages);
		...
		_beforeClose: function () {
			if (ResponsiveElementorSitesAdmin.action == 'insert') {
				$elscope.find('.rst-library-template-insert').removeClass('installing');
				$elscope.find('.rst-library-template-insert').text('Imported');
				$elscope.find('.rst-library-template-insert').addClass('action-done');

				if ($elscope.find('.rst-sites-floating-notice-wrap').hasClass('slide-in')) {

					$elscope.find('.rst-sites-floating-notice-wrap').removeClass('slide-in');
					$elscope.find('.rst-sites-floating-notice-wrap').addClass('slide-out');

					setTimeout(function () {
						$elscope.find('.rst-sites-floating-notice-wrap').removeClass('slide-out');
					}, 200);
				}
			}
		},
		...
		_goBack: function (e) {
			...
			$elscope.find('#rst-sites-floating-notice-wrap-id.error').hide();

			$elscope.find('.responsive-sites-step-1-wrap').show();
			$elscope.find('.responsive-preview-actions-wrap').remove();

			$elscope.find('.rst-template-library-toolbar').show();
			$elscope.find('.rst-sites-modal__header').removeClass('rst-preview-mode');
			...
			$elscope.find('.responsive-sites-content-wrap').trigger('scroll');
		},
		...
		_goStep2: function (e) {
			...
			ResponsiveElementorSitesAdmin.site_id = $elscope.find('#responsive-blocks').data('site-id');
			ResponsiveElementorSitesAdmin.page_id = '';
		...
		_toggleTooltip: function (e) {
			...
			var wrap = $elscope.find('.rst-tooltip-wrap');

			if (wrap.hasClass('rst-show-tooltip')) {
				$elscope.find('.rst-tooltip-wrap').removeClass('rst-show-tooltip');
			} else {
				$elscope.find('.rst-tooltip-wrap').addClass('rst-show-tooltip');
			}
		},
		...
		_preview: function (e) {
			...
			$elscope.find('.rst-template-library-toolbar').hide();
			$elscope.find('.rst-sites-modal__header').removeClass('rst-preview-mode');
		...
		_requiredPluginsMarkup: function (requiredPlugins) {
			...
			if (
				...
				if (!responsiveElementorSites.license_status) {
					...
					$elscope.find('.rst-tooltip-wrap').css('opacity', 1);
					$elscope.find('.responsive-sites-tooltip').css('opacity', 1);
					...
					$elscope.find('.responsive-sites-import-template-action > div').removeClass('disabled');
					return;
				}
			...
			$.ajax({
				...
				.done(function (response) {
					...
					if (false === response.success) {
						...
						$elscope = $('#rst-sites-modal');
						$elscope.find('#rst-sites-floating-notice-wrap-id').show().removeClass('error');
						$elscope.find('#rst-sites-floating-notice-wrap-id .rst-sites-floating-notice').show().html('<span class="message">Insufficient Permission. Please contact your Super Admin to allow the install required plugin permissions.<span>');
						$elscope.find('#rst-sites-floating-notice-wrap-id').addClass('error slide-in').removeClass('refreshed-notice');
		...
		_step2: function (e) {
			...
			$elscope.find('.rst-sites-modal__header').addClass('rst-preview-mode');
			...
			$elscope.find('.responsive-sites-step-1-wrap').hide();
			...
			if (undefined !== data && undefined !== data['post-meta']['_elementor_data']) {
				...
				$.ajax({
					...
					.done(function (response) {
						...
						$elscope.find('.responsive-sites-content-wrap').removeClass('processing');
		...
		_addSites: function (data) {
			...
			if (data) {
				...
				$elscope.find('.dialog-lightbox-content').html(wp.template('responsive-sites-no-sites'));
			}
		},
